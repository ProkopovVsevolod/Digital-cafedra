...
export namespace Transport {
    ...
    export class Vehicle implements IVehicle {
        ...
        print(): string {
            const returnData: string = `VIN: ${this.vin} Владелец: ${this.owner.name + " " +  this.owner.sername} Номер: ${this.regNum}`;
    ...
    export class VehicleStorage implements IVehicleStorage<IVehicle> {
        ...
        //Задание 1 
        getSortedByModel(): void {
            console.log("Хранилище до сортировки: ", this.getVehile());
            this.vehicleStorage.sort((a, b) => {
                if (a.model.toLowerCase() < b.model.toLowerCase()) return -1;  
                if (a.model.toLowerCase() == b.model.toLowerCase()) return 0;
                if (a.model.toLowerCase() > b.model.toLowerCase()) return 1;
            }) 
            console.log("Хранилище после сортировки: ", this.getVehile());
        }

        getVehicleByModelEnd(endOfModel:string) {
            const result = this.vehicleStorage.filter(vehicle => {
                const modelLower = vehicle.model.toLowerCase();
                const endOfModelLower = endOfModel.toLowerCase();
